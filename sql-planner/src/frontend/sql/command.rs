...
fn parse_with_options(options: &mut CopyOptions, pair: Pair<'_, Rule>) -> Result<(), SbroadError> {
    ...
    for option in pair.into_inner() {
        let rule = option.as_rule();
        match rule {
            Rule::CopyFormatOption => {
                if seen_format {
                    return Err(duplicate_copy_option(rule));
                }
                options.format = parse_copy_format(&parse_option_value(option))?;
                seen_format = true;
            }
            Rule::CopyDelimiterOption => {
                if options.delimiter.is_some() {
                    return Err(duplicate_copy_option(rule));
                }
                options.delimiter = Some(parse_option_value(option));
            }
            Rule::CopyNullOption => {
                if options.null_string.is_some() {
                    return Err(duplicate_copy_option(rule));
                }
                options.null_string = Some(parse_option_value(option));
            }
            ...
            Rule::CopySessionFlushRowsOption => {
                if options.session_flush_rows.is_some() {
                    return Err(duplicate_copy_option(rule));
                }
            ...
            Rule::CopyDestinationFlushRowsOption => {
                if options.destination_flush_rows.is_some() {
                    return Err(duplicate_copy_option(rule));
                }
            ...
            Rule::CopySessionFlushBytesOption => {
                if options.session_flush_bytes.is_some() {
                    return Err(duplicate_copy_option(rule));
                }
            ...
            Rule::CopyDestinationFlushBytesOption => {
                if options.destination_flush_bytes.is_some() {
                    return Err(duplicate_copy_option(rule));
                }
...
fn duplicate_copy_option(rule: Rule) -> SbroadError {
    SbroadError::Invalid(
        Entity::Query,
        Some(format_smolstr!(
            "duplicate COPY option: {}",
            copy_option_name(rule)
        )),
    )
}

fn copy_option_name(rule: Rule) -> &'static str {
    match rule {
        Rule::CopyFormatOption => "FORMAT",
        Rule::CopyDelimiterOption => "DELIMITER",
        Rule::CopyNullOption => "NULL",
        Rule::CopyHeaderOption => "HEADER",
        Rule::CopySessionFlushRowsOption => "SESSION_FLUSH_ROWS",
        Rule::CopyDestinationFlushRowsOption => "DESTINATION_FLUSH_ROWS",
        Rule::CopySessionFlushBytesOption => "SESSION_FLUSH_BYTES",
        Rule::CopyDestinationFlushBytesOption => "DESTINATION_FLUSH_BYTES",
        Rule::CopyRowBytesOption => "ROW_BYTES",
        _ => unreachable!("unexpected COPY option rule"),
    }
}
...
mod tests {
    ...
    #[test]
    fn rejects_duplicate_copy_option_with_value() {
        let error = parse_command(r#"COPY "t" FROM STDIN WITH (DELIMITER '|', DELIMITER ',')"#)
            .expect_err("duplicate COPY delimiter should be rejected");

        assert_eq!(
            error.to_string(),
            "invalid query: duplicate COPY option: DELIMITER"
        );
    }

    #[test]
    fn rejects_duplicate_copy_option_with_default_value() {
        let error = parse_command(r#"COPY "t" FROM STDIN WITH (HEADER, HEADER false)"#)
            .expect_err("duplicate COPY header should be rejected");

        assert_eq!(
            error.to_string(),
            "invalid query: duplicate COPY option: HEADER"
        );
    }
